...
import { Invite, InviteResponse } from './messages.js'
...
/** @typedef {Peer['info']} PeerInfo */
/** @typedef {'connecting' | 'connected' | 'disconnected'} PeerState */

/**
 * @template ValueType
 * @typedef {object} DeferredPromise
 * @property {Promise<ValueType>} promise
...
class Peer {
  ...
  #assertConnected() {
    ...
    throw new PeerDisconnectedError('Peer disconnected')
  }
}
 ...
 * @property {(invite: Invite) => void} invite Emitted when an invite is received
 */
...
export class MapeoRPC extends TypedEmitter {
  ...
  async invite(peerId, { timeout = 60000, ...invite }) {
    ...
    const promise = new Promise((origResolve, origReject) => {
      ...
      const pending = this.#pendingInvites.get(projectId) || []
      this.#pendingInvites.set(projectId, pending)
  ...
  connect (stream) {
    ...
    const protomux = Protomux.isProtomux(stream.userData)
      ? stream.userData
  ...
  #emitPeers () {
    const peerInfos = [...this.#peers.values()].map(peer => peer.info)
    this.emit('peers', peerInfos)
  }
  ...
  #handleMessage (peerPublicKey, type, value) {
    ...
    switch (type) {
      ...
      case 'InviteResponse': {
        ...
        this.#pendingInvites.set(projectId, [])
        break
      }
      default:
