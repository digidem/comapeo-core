...
import { parseSqlResponse } from './utils.js'
...
export class Sqlite {
   ...
   */
  constructor(filepathOrDatabase, options = {}) {
    this.db =
      typeof filepathOrDatabase === 'string'
        ? BetterSqlite(this.#filepath, options)
        : filepathOrDatabase
  ...
  query(sql, params) {
    ...
    return rows.map((row) => {
      for (const [key, value] of Object.entries(row)) {
        if (['links', 'forks', 'tags', 'refs', 'attachments'].includes(key)) {
