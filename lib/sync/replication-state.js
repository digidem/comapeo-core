...
export class CoreReplicationState extends TypedEmitter {
  ...
  constructor(options) {
    ...
    this.#core.on('download', async (index, byteLength, from) => {
      const peerState = this.#getPeerState(from.remotePublicKey)
      ...
      state.want--
      state.have++
      state.unavailable = peerState.unavailable
  ...
  #getLocalState() {
    return this.#state.get(this.coreId) || createState()
  }
  ...
  #getPeerState(peerPublicKey) {
    return this.#state.get(keyToId(peerPublicKey)) || createState({ remote: true })
  }
