...
/** @type {import('fastify').FastifyPluginAsync<BlobServerPluginOpts>} */
...
/** @type {import('fastify').FastifyPluginAsync<Omit<BlobServerPluginOpts, 'prefix'>>} */
async function routes(fastify, options) {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      if (!blobs) {
        reply.code(500)
        throw new Error('Hyperdrive instance has no blobs instance')
      }

      // TODO: Need to update Hyperdrive to latest and then pass `wait: false` here
