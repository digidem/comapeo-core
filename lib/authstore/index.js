...
export class AuthStore {
  ...
  async ready() {
    ...
    for (const dataType of this.#datastore.dataTypes) {
      /** @ts-ignore */
  ...
  async initProjectCreator() {
    ...
    await this.addDevice({ identityId: this.id })
  }
  ...
  async setCoreOwner(coreKey) {
    ...
    const authorIndex = await this.getAuthorIndex(this.id)
    const deviceIndex = await this.getDeviceIndex(this.id)
  ...
  async addDevice(options) {
    ...
    const authorRole = await this.getRole({ identityId: this.id })
  ...
  getCoreOwnershipStatementByCoreId(coreId) {
    return /** @type {CoreOwnershipStatement} */ (
      this.#sqlite.get(
        `SELECT * FROM coreOwnership WHERE coreId = '${coreId}'`
      )
    )
  }
