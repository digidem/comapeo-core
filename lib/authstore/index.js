...
export class Authstore {
  ...
  /** @type {import('../core-manager/index.js').CoreManager} */
  #coreManager
  ...
  async ready() {
    ...
    this.#keyPair = this.#coreManager.getWriterCore('auth').core?.keyPair
    await this.#init()
  }
  ...
  async getBlockByVersion(version) {
    ...
    return /** @type {CoreOwnershipStatement|RoleStatement|DeviceStatement} */ (
      dataType.decode(block, { coreId: core.key, blockIndex })
    )
  }
  ...
  async *getBlocks() {
    for (const core of this.cores) {
      ...
      if (core.length) {
        let index = 0
        for await (const buf of core.createReadStream({ start: 0 })) {
