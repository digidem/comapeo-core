...
export class AuthStore {
  ...
  async verifyCoreOwner(statement) {
    const { id, authorId, coreId, signature, authorIndex, deviceIndex } = statement
  ...
  async verifyProjectCreator(roleStatement) {
    const { id, authorId, signature, authorIndex, deviceIndex, links } = roleStatement
  ...
  getCoreOwnershipStatementByVersion(version) {
    return /** @type {CoreOwnershipStatement} */ (
      this.#sqlite.get(
        `SELECT * FROM core_ownership WHERE version = '${version}'`
      )
    )
  }
  ...
  getRoleStatementByVersion(version) {
    return /** @type {RoleStatement} */ (
      this.#sqlite.get(`SELECT * FROM roles WHERE version = '${version}'`)
    )
  }
  ...
  getDeviceStatementByVersion(version) {
    return /** @type {DeviceStatement} */ (
      this.#sqlite.get(`SELECT * FROM devices WHERE version = '${version}'`)
    )
  }
  ...
  async verifyDevice(statement) {
    const { id, signature, authorId, action, authorIndex, deviceIndex, links } = statement
  ...
  async verifyRole(options) {
    const { id, authorId, role, signature, links, authorIndex, deviceIndex } = options
