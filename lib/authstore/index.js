...
const capabilitiesList = ['none', 'member', 'admin', 'creator']
...
export class AuthStore {
  ...
  constructor(options) {
    ...
    this.indexer = new Indexer({
      ...
      extraColumns: 'capability TEXT, identityPublicKey TEXT, blockIndex INTEGER',
    })
  ...
  async validateParentStatement (statement) {
    const capability = statement.capability
    const capabilityIndex = capabilitiesList.indexOf(capability)
    ...
    const parentStatement = await this.getBlockByVersion(capability.links[0])
    const parentCapability = parentStatement.capability

    if (parentCapability === 'none' || parentCapability === 'member') {
      return false
    }

    if (parentCapability === 'admin' || parentCapability === 'creator') {
      if (!parentStatement.links.length) {
        return true
      }

      return this.validateParentStatement(parentStatement)
    }
