...
export class AuthStore {
  ...
  async verifyCoreOwner(statement) {
    const { id, authorId, coreId, signature, authorIndex, deviceIndex, timestamp } =
      statement
  ...
  async setProjectCreator() {
    ...
    if (authorIndex !== 1 && deviceIndex !== 1) {
      throw new Error('Project creator record must be created by the first device')
    }
  ...
  async verifyProjectCreator(roleStatement) {
    const { id, authorId, signature, authorIndex, deviceIndex, timestamp, links } =
      roleStatement
  ...
  async verifyDevice(statement) {
    const { id, signature, authorId, action, authorIndex, deviceIndex, timestamp, links } =
      statement
  ...
  async verifyRole(options) {
    const { id, authorId, role, signature, links, authorIndex, deviceIndex, timestamp } =
      options
