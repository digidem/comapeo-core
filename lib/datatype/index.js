...
export class DataType {
  ...
  #decode = function defaultDecode(block) {
    ...
    if (blockPrefix !== this.blockPrefix) {
      throw new Error(
        `Data processed by ${this.name} DataType with hex identifier ${blockPrefix} found, expected ${this.blockPrefix}`
      )
    }
  ...
  validate(doc) {
    ...
    if (!valid) {
      // @ts-ignore: #validate.errors is always defined if valid is false
      throw new Error(
