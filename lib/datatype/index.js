...
export class DataType {
   ...
   * @param {Core} options.core the local writer hypercore
  ...
  async #shouldUpdate (doc) {
    ...
    if (!links || !links.length) {
      throw new Error('Cannot update a doc without a link to the previous version')
    }

    for (const version of links) {
      const block = await this.getByVersion(version)

      // TODO: validate that doc and block are the same dataType
