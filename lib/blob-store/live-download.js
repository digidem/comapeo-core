...
export class LiveDownload extends TypedEmitter {
   ...
   */
  constructor (drives, emitter, { folder = '/', signal, recursive }) {
    super()
    this.#signal = signal
...
export class DriveLiveDownload extends TypedEmitter {
   ...
   * @param {boolean} [options.recursive]
   */
  constructor (drive, { folder = '/', signal, recursive } = {}) {
    super()
    this.#drive = drive
    this.#folder = folder
    this.#signal = signal
  ...
  async #start () {
    ...
    if (!blobs) {
      ...
      // In the case of a drive that was added by its key, but never replicated,
      // then we will never get blobs, because it is read from the header on the
      // db feed. This messy code below allows this to be cancelled.
      /** @type {Promise<void>} */
      const abortPromise = new Promise(res => {
        if (!this.#signal) return
        if (this.#signal.aborted) return res()
        this.#signal.addEventListener('abort', () => res(), { once: true})
      })
      const result = await Promise.race([
        once(this.#drive, 'blobs'),
        abortPromise
      ])
      blobs = result?.[0]
    }
    /* c8 ignore next */
    if (this.#signal?.aborted || !blobs) return // Can't get here in tests
    let seq = 0
