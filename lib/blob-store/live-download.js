...
export class LiveDownload extends TypedEmitter {
   ...
   * @param {import('../types').BlobDownloadSelection} [options.filter] Filter blobs of specific types and/or sizes to download
 ...
 *
 * @param {import('../types').BlobDownloadSelection} [filter]
 * @returns {string[]} array of folders that match the filter
 */
function filterToFolders (filter) {
  if (!filter) return ['/']
  const folders = []
  const types = filter.types || ['audio', 'photo', 'video']
  const sizes = filter.sizes || ['original', 'preview', 'thumbnail']
  for (const type of types) {
    for (const size of sizes) {
      // For now, only photos support non-original sizes
      if (type !== 'photo' && size !== 'original') continue
      folders.push(makePath({ type, size }))
    }
  }
  return folders
}
...
/** @param {Pick<import('../types').BlobId, 'type' | 'size'>} opts */
function makePath ({ type, size }) {
  return `/${type}/${size}`
}
