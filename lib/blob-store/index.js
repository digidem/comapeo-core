...
export class BlobStore {
   ...
   * @param {BlobId} blobId
   * @param {object} opts
   * @param {boolean} [opts.wait=false] Set to `true` to wait for a blob to download, otherwise will return `null` if not available locally
   * @param {number} [opts.timeout] Optional timeout to wait for a blob to download
   */
  async get ({ type, size, name, driveId }, { wait = false, timeout } = {}) {
    ...
    return drive.get(path, { wait, timeout })
  }

  /**
   * Download blobs from all drives, optionally filtering particular blob types
   * or blob sizes. Download will be 'live' and will continue downloading new
   * data as it becomes available from any replicating drive.
   *
   * @param {import('../types').BlobDownloadSelection} [filter] Filter particular blob types and/or sizes to download
   * @param {object} options
