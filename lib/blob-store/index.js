...
export class BlobStore {
  ...
  /**
   * Download a folder path from all drives. Download will be 'live' and will
   * continue downloading new data as it becomes available from any replicating
   * drive.
   *
   * @param {string} [folder='/'] Folder path to download
   * @param {object} options
   * @param {AbortSignal} [options.signal] Optional AbortSignal to cancel in-progress download
   * @param {boolean} [options.recursive=true] Recursively download subfolders too, default=true
   * @returns EventEmitter with `.state` propery, emits `state` with new state when it updates
   */
  download(folder = '/', { signal, recursive=true } = {}) {
    return new LiveDownload(this.#hyperdrives.values(), this.#driveEmitter, { folder, signal, recursive })
  }
