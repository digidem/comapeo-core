...
/** @typedef {DuplexStream & { noiseStream: DuplexStream }} NoiseStream */
/** @typedef {DuplexStream & { noiseStream: DuplexStream & { userData: any }}} ProtocolStream */
/**
...
export class CoreManager extends TypedEmitter {
  ...
  constructor ({ db, keyManager, projectKey, projectSecretKey, storage }) {
    ...
    assert(
      !projectSecretKey || projectSecretKey.length === 32,
      'project owner core private key must be 32-byte buffer'
    )
