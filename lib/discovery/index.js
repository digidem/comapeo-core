...
export class Discovery extends TypedEmitter {
   ...
   *
   * // Create topic name to be joined (see footnote 1)
   * const topicName = 'my-topic'
   *
   * discover.join(topicName)
   *
   * console.log(discover.topics) // prints out `[Topic]`, where `Topic` is a class instance containing information about the topic joined
   * ```
   *
   * Footnotes:
   *
   * 1. To create a secure topic name, refer to `KeyManager`, `createIdentityKeys`, and `getHypercoreKeypair` in the [`@mapeo/crypto`](https://github.codigidem/mapeo-crypto) docs.
   */
  get topics() {
   ...
   * const topic = Buffer.from('peers-example')
   *
   ...
   * Footnotes:
   *
   * 1. See [Hyperswarm](https://github.com/hyperswarm/hyperswarm) docs for available DHT options.
   */
   ...
   *
   * ```js
   * const topic = Buffer.from('getPeersByTopic-example')
   *
   ...
   *
   * ```js
   * const topic = Buffer.from('leave-example')
   *
   * await discover.join(topic)
   *
   ...
   *
   * ```js
   * const topic = Buffer.from('leavePeer-example')
   *
   * discover.on('connection', (connection, peer) => {
   *  console.log('Connected to peer', peer.identityPublicKey)
   *  console.log(discover.peers) // Prints `[Peer]` with newly connected `peer`
   *
   *  setTimeout(() => {
   *    discover.leavePeer(peer.identityPublicKey).then(() => {
   *      console.log('Disconnected from peer', peer.identityPublicKey)
   ...
   *
   * ```js
   * const topic = Buffer.from('destroy-example')
   *
   * await discover.ready()
   * await discover.join(topic)
   *
...
export class PeerInfo extends TypedEmitter {
   ...
   *
   * ```js
   * const topic = Buffer.from('my-topic').toString('hex')
   * const pubKey = Buffer.from('my-public-key').toString('hex')
   *
   * const peer = new PeerInfo({
   *  topics: [topic],
   ...
   * })
   *
   * const topic = Buffer.from('addTopic-example').toString('hex') // '6d792d746f706963'
   *
   * peer.addTopic(topic)
   *
   * console.log(peer.topics) // Prints `['6d792d746f706963']`
   *
   * // Adding the same topic again is idempotent
   * peer.addTopic(topic)
   *
   * console.log(peer.topics) // Still prints `['6d792d746f706963']`
   * ```
   */
  addTopic(topic) {
   ...
   * })
   *
   * const topicFoo = Buffer.from('foo').toString('hex') // '666f6f'
   * const topicBar = Buffer.from('bar').toString('hex') // '626172'
   *
   * peer.addTopics([topicFoo, topicBar])
   *
   * console.log(peer.topics) // Prints `['666f6f', '626172']`
   *
   * // Adding the same topic(s) again is idempotent
   * peer.addTopics([topicBar, topicFoo])
   *
   * console.log(peer.topics) // Still prints `['666f6f', '626172']`
   * ```
   */
  addTopics(topics) {
   ...
   *
   * ```js
   * const topic = Buffer.from('removeTopic-example').toString('hex') // '72656d6f7665546f7069632d6578616d706c65'
   *
   * const peer = new PeerInfo({
   *  topics: [topic],
   *  // For the sake of brevity, specify the rest of the contructor options here...
   * })
   *
   * console.log(peer.topics) // Prints `['72656d6f7665546f7069632d6578616d706c65']`
   *
   ...
   * console.log(peer.topics) // Prints `[]`
   *
   * const topic = Buffer.from('topics-example').toString('hex') // '746f706963732d6578616d706c65'
   *
   * peer.addTopic(topic)
   *
   * console.log(peer.topics) // Prints `['746f706963732d6578616d706c65']`
   * ```
   */
  get topics() {
    return Array.from(this.#topics.values())
   ...
   *
   * ```js
   * const pubKey = Buffer.from('my-public-key').toString('hex') // '6d792d7075626c69632d6b6579'
   * const topic = Buffer.from('toJSON-example').toString('hex') // '746f4a534f4e2d6578616d706c65'
   *
   * const peer = new PeerInfo({
   *  identityPublicKey: pubKey,
   *  topics: [topic],
   ...
   * })
   *
   * console.log(peer.toJSON()) // Prints { topics: [ '746f4a534f4e2d6578616d706c65' ], identityPublicKey: '6d792d7075626c69632d6b6579', host: 'some.address.lan.local', port: 12345 }
   * ```
   */
  toJSON() {
    return {
...
export class Topic extends TypedEmitter {
   ...
   * ```js
   * const topic = new Topic({
   *  topicBuffer: Buffer.from('my-topic'),
   *  dhtStatus: 'joining',
   *  mdnsStatus: 'joining',
   ...
   * const topic = new Topic({
   *  topicBuffer: Buffer.from('updateStatus-example'),
   *  dhtStatus: 'joined',
   *  mdnsStatus: 'closed',
   *  mdns: new MdnsDiscovery(),
   *  dht: new Hyperswarm(),
