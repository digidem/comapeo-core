...
export class Discovery extends TypedEmitter {
   ...
   * const km = new KeyManager(KeyManager.generateIdentityKey())
   * const keyPair = km.getHypercoreKeypair(topicName, Buffer.alloc(32, 0))
   *
   * const discover = new Discover({
   *  identityKeyPair: km.getIdentityKeypair()
   * })
   *
   * await discover.ready()
   *
   * discover.join(keyPair.identityPublicKey)
   *
   ...
   * })
   *
   * // Create a shared topic name (usually would use @mapeo/crypto)
   * const topic = 'peers-example'
   *
