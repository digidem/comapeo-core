 ...
 * @property {(topicStatus: { topic: TopicId, mdns: TopicServiceStatus, dht: TopicServiceStatus }) => void} topicStatus
...
export class Discovery extends TypedEmitter {
  ...
  constructor(options) {
    ...
    if (this.mdnsActive) {
      ...
      this.#tcp?.on('error', error => {
        console.error('error instanciating mdns client', error)
      })
      this.mdns = mdns
    }
  ...
  async _onMdnsConnection(socket) {
    // end connection if its from a non-local node
    const address = socket.remoteAddress?.replace(/^.*:/, '')
    if(!bogon.isPrivate(address)) socket.end()
  ...
  async join(topicBuffer, options = {}) {
    const { mdns = this.mdnsActive, dht = this.dhtActive } = options
   ...
   */
  async _startServer() {
    await promisify(this.#tcp?.listen).call(this.#tcp, 0, '0.0.0.0')
...
export class PeerInfo extends TypedEmitter {
   ...
   * @property {IdentityId} identityPublicKey
   ...
   *
   * @param {Object} options
   * @param {IdentityId} [options.identityPublicKey]
...
export class Topic extends TypedEmitter {
  /**
   * @param {Object} options
   * @param {TopicKey} options.topicBuffer
