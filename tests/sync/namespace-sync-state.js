...
import { CoreSyncState } from '../../src/sync/core-sync-state.js'
...
test.solo('access peer state', async function (t) {
  t.plan(12)
  ...
  const syncState = new NamespaceSyncState({
    coreManager: cm1,
    namespace: 'auth',
    stateEventThrottleMs: 0,
  })
  ...
  setTimeout(async function append() {
    await writer.core.append('a')
    if (writer.core.length < 6) setTimeout(append, 100)
  }, 100)

  const reader = cm2.getCoreByKey(writer.core.key)

  syncState.on('state', async function handler({ localState }) {
    if (writer.core.length === 6 && reader.contiguousLength === 0) {
      reader.download({ start: 0, end: 6 })
    } else if (1) {
    }
  })
})
