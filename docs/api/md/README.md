**API** • **Docs**

***

# API

## Modules

- [\<internal\>](-internal-/README.md)

## Namespaces

- [BlobApi](namespaces/BlobApi/README.md)
- [IconApi](namespaces/IconApi/README.md)
- [InviteApi](namespaces/InviteApi/README.md)
- [MemberApi](namespaces/MemberApi/README.md)

## Classes

- [FastifyController](classes/FastifyController.md)
- [MapeoManager](classes/MapeoManager.md)

## Type Aliases

- [EditableProjectSettings](type-aliases/EditableProjectSettings.md)
- [MapeoProject](type-aliases/MapeoProject.md)
- [MapShare](type-aliases/MapShare.md)

## Variables

- [roles](variables/roles.md)

## Functions

- [CoMapeoMapsFastifyPlugin](functions/CoMapeoMapsFastifyPlugin.md)
- [replicateProject](functions/replicateProject.md)
